...
> 状态：已确认  
> 锁定：建产品后留产品详情 · 需求/项目/迭代/缺陷列表共享可选产品筛选 · 迭代仅拉入 `type=story` 且 `status=active`
...
| `/bugs/:id/edit` | 编辑缺陷（居中 Modal） | `bug.edit` | F-BUG-03 |
...
6. **抽屉 vs 弹窗**：产品/项目/迭代及需求**详情**/缺陷**详情**用**右侧抽屉**；需求/缺陷**新建**用居中大 Modal；分配角色、解决缺陷、关联需求等次要操作用**弹窗**；系统用户/角色/菜单 CRUD 继续用弹窗。  
...
**筛选：** 产品（可选，可按名称搜索，跨页共享，见 §0.3.7）；type、status、指派人、关键词(title)

**列：** ID、标题、产品、类型(planning/story)、优先级、状态、估算、指派人、创建人、附件数、更新时间、操作  

**排序：** 点击表头「类型 / 优先级 / 状态」在无 → 正序 → 倒序间切换；同时仅一列生效；默认按 ID 倒序

**按钮：** 新建 → `/stories/new`（有选中产品时带 `?productId=`）；行内查看/编辑/删除

**建议默认：** 从产品详情点「需求」进入时带 `?productId=`；未选产品时加载全部需求。
...
**筛选：** 产品（可选，可按名称搜索，跨页共享）；status、关键词 

**列：** ID、名称、代号、所属产品、状态、PM、创建人、开始/结束、迭代数、操作  
...
**筛选：** 产品（可选，可按名称搜索，跨页共享）、项目、status  

**列：** ID、名称、项目、产品(冗余展示)、状态、创建人、开始/结束、需求数、操作  
...
**列：** ID、标题、产品、严重程度、优先级、状态、指派人、创建人、创建日期、关联需求、附件数、操作  

**排序：** 点击表头「严重程度 / 优先级 / 状态 / 创建日期」在无 → 正序 → 倒序间切换；同时仅一列生效；默认按 ID 倒序  
...
| assigned_to | | 可指派他人 |
| title | ✓ | 整行 |
| severity / pri | | 默认 3；同行 |
| project_id / sprint_id | | 级联；同行 |
| story_id | | |
| steps | | 所见即所得；可插图片/mp4 |
| 附件 | | 新建时可多选；保存后上传 |

### 7.3 编辑 `/bugs/:id/edit`

居中大 Modal；可改标题、步骤、严重程度、优先级、项目/迭代/需求、指派；产品不可改。编辑态含附件面板。

### 7.4 详情 `/bugs/:id`

右侧抽屉。

**操作：** 编辑、解决、关闭、激活（弹窗填说明+截图，写入备注）、添加备注、删除(仅 active)

**区块：** 重现步骤(HTML)、附件（可上传/删除）、备注列表

**解决弹窗：** resolution*、解决备注、指派给（默认创建人，可改）
